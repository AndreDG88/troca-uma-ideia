...
def test_authenticated_user_can_create_tweet():
    ...
    response = client.post("/api/tweets/", {"content": "Um tweet via API"}, format="json")
...
def test_unauthenticated_user_cannot_create_tweet():
    ...
    response = client.post("/api/tweets/", {"content": "Tentativa não autenticada"}, format="json")
...
def test_user_can_update_own_tweet():
    ...
    response = client.put(f"/api/tweets/{tweet.id}/", {"content": "Editado"}, format="json")
    assert response.status_code == 200
...
def test_user_cannot_update_other_user_tweet():
    ...
    response = client.put(f"/api/tweets/{tweet.id}/", {"content": "Tentativa de edição"}, format="json")
